...
st.markdown("""
    ...
    .stButton>button { width: 100%; font-weight: bold; height: 3.5em; background-color: #FF4B4B; color: white; border-radius: 12px; border: none; font-size: 18px; }
    .stButton>button:hover { background-color: #D43F3F; border: none; }
    </style>
""", unsafe_allow_html=True)

# הצגת התמונה - מעודכן ל-ELLA2.jpg
image_name = "ELLA2.jpg" 

if os.path.exists(image_name):
    st.image(image_name, use_container_width=True)
else:
    st.info(f"💡 מחפש תמונה בשם {image_name}... וודא שהיא בתיקייה הראשית ב-GitHub.")
...
st.subheader("גרסת ELLAGAME - המנוע הרשמי")
